...
if __name__ == "__main__":
    ...
    melk_ckpt_name = None
    name = None
    create_logdir_from_config = False
    if opt.resume:
        ...
        if not opt.no_date:
            ...
            if nowname.startswith("_"):
                nowname = nowname[1:]
        create_logdir_from_config = True
    ...
    try:
        ...
        logdir = os.path.join(opt.logdir, config['logdir'])
        ...
        # def disable_model_checkpointing(model):
        #     # Many blocks expose either `.checkpoint` or `.use_checkpoint`
        ...
        # disable_model_checkpointing(model)
        # run
        if opt.train:
            try:
