...
class DiffusionEngine(pl.LightningModule):
    def __init__(
        ...
        # BERTScore config
        self.bertscore_model_type = "microsoft/deberta-base"  # or "roberta-large"
        self.bertscore_lang = "en"

        # RL / DDPO configs stay as is
        self.use_ddpo = True
        self.ddpo_lambda = 0.1
        self.ddpo_steps = 2
        self.ddpo_frames_for_reward = 3
        self.ddpo_eta = 1.0
        self.ddpo_every = 4
        self.adv_ema = EMA(beta=0.9)

        self.num_views = 3
        self.frames_per_view = 8
        self.num_video_frames = 25

        self.rollout_seq_mb = 2
        self.ddpo_seq_mb = 2
        self.reward_decode_mb = 4
        self.reward_down_hw = 160
        self.reward_keyframe_index = 4
        self.ddpo_amp_dtype = "fp16"
    ...
    def _blip_move_to(self, target) -> torch.device:
        """
        Move BLIP model to `target` device.
        """
        dev = target if isinstance(target, torch.device) else torch.device(target)
        self.blip_model.to(dev)
        return dev

    def _cr_offload_cpu(self):
        """Offload CycleReward to CPU and free CUDA cache."""
        import torch
        self._blip_move_to(torch.device("cpu"))
    ...
    def get_cyclereward_loss(
        ...
        rm_device = self._blip_move_to(device)  # <<< FIX: no extra self
        ...
        with torch.inference_mode():
            for s in range(0, len(decode_indices), decode_mb):
                ...
                preds = self.blip_processor.batch_decode(
                ...
                # 5) BERTScore between preds (hypothesis) and refs (reference)
                device_str = str(rm_device) if rm_device.type == "cuda" else "cpu"
                _, _, F1 = bertscore_score(
                    preds,
                    caps,
                    model_type=self.bertscore_model_type,  # e.g. "roberta-base"
                    lang=self.bertscore_lang,              # e.g. "en"
                    verbose=False,
                    device=device_str
                )
                sc = F1.to(device).view(-1)  # (mb,)
