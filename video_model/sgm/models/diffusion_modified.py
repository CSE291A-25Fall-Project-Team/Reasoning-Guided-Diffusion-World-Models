...
class DiffusionEngine(pl.LightningModule):
    ...
    ###### RL LOSS ##########
    def awr_training_step(self, batch, beta: float = 0.5, ema: float = 0.9):
        # 1) sample a rollout video and get reward
        c, uc = self.conditioner.get_unconditional_conditioning(
            batch,
            force_uc_zero_embeddings=self.conditioner.input_keys if len(self.conditioner.embedders) > 0 else [],
        )
        sampler_kwargs = {
        ...
        # sample video latents from current policy
        latents = self.sample(
            cond=c,
            uc=uc,
            batch_size=B,
        ...
        videos = self.decode_first_stage(latents)      # (B*T, C, H, W)
        T = batch["num_video_frames"]
        videos = videos.view(B, T, *videos.shape[1:])  # (B, T, C, H, W)
        videos_vlm = (videos.clamp(-1, 1) + 1) / 2.0   # to [0,1]
        videos_np = videos_vlm.permute(0, 1, 3, 4, 2).detach().cpu().numpy()

        rewards = []
        for i in range(B):
            reward_info = self.critic.score_frames(
                frames=videos_np[i],
                question="What is the robot doing in the video?",   
                reference=batch["instruction"][i],    # change this to steps
            )
            rewards.append(reward_info["reward"])
        reward = torch.tensor(rewards, device=self.device)

        # # critic reward (B,)
        # texts = batch["instruction"]   ## change this to steps
        # with torch.no_grad():
        #     reward = self.critic.score(videos_vlm, texts)

        # running baseline
        baseline = getattr(self, "_reward_ma", 0.0)
        new_baseline = ema * baseline + (1.0 - ema) * reward.mean().item()
        self._reward_ma = new_baseline
        adv = reward - new_baseline   # (B,)

        # 2) normal per-sample diffusion loss (this is your -log p)
        ...
        loss_per_sample = loss.view(B, T).mean(dim=1)

        # 3) advantage weights
        weights = torch.exp((adv / beta).clamp(max=5.0))    # (B,)
        weights = weights / (weights.mean().detach() + 1e-8)

        awr_loss = (weights.detach() * loss_per_sample).mean()
        ...
        awr_loss_dict = {
                "loss": awr_loss.detach(),
                "reward": reward.mean().detach(),
                "awr_weight_mean": weights.mean().detach(),
            },
            # prog_bar=True,
            # on_step=True,
            # on_epoch=False,
    ...
    def training_step(self, batch, batch_idx):
        ### Adding RL loss part
        if self.global_step % 3 == 0:     # change this
            loss, loss_dict = self.awr_training_step(batch, batch_idx, beta=0.5)
            print(f"Step {self.global_step}: AWR loss {loss.item():.4f}")
    ...
    def instantiate_optimizer_from_config(self, params, lr, cfg):
        print(f"Instantiating optimizer from config: {cfg}")
    ...
    def configure_optimizers(self):
        ...
        modules = list(vu.input_blocks) + list(vu.output_blocks)
        ...
        print(f"h_pose_indicies: {vu.h_pose_indicies}")

        ### Fine-tune specific blocks that go into action generation model
        for idx in vu.h_pose_indicies:
            print(f"Fine-tuning block {idx} for action generation")
            block = modules[idx]
            print(block)

            for p in block.parameters():
                p.requires_grad = True
                param_groups.append({"params": p, "lr": lr})   # or lr * some_factor
