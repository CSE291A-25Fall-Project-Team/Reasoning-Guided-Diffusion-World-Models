...
class DiffusionEngine(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # 1) VideoUNet: ONLY the blocks used for ActionNet (h_pose_indicies)
        if self.grad_config.get('train_video_unet', False):
            vu = find_videounet(self.model)
            blocks = list(vu.input_blocks) + list(vu.output_blocks)

            # use the indices defined on the UNet if present; otherwise default to your list
            h_pose_indices = getattr(vu, "h_pose_indicies", [9, 13, 16, 19, 22])

            selected_params, seen = [], set()
            for idx in h_pose_indices:
                if idx < 0 or idx >= len(blocks):
                    raise IndexError(f"h_pose_indicies contains {idx}, but blocks has length {len(blocks)}")
                for p in blocks[idx].parameters():
                    ...
                    seen.add(id(p))
            print("length of selected params", len(selected_params))
            if selected_params:
                param_groups.append({"params": selected_params, "lr": lr})  # same LR as your original 'video_unet'

        # 2) vision_encoder (3D ResNet used by Action branch)
        if self.grad_config.get('train_vision_encoder', False):
            ve_params = []
        ...
        # 3) pose_pred_net (Action UNet1D)
        if self.grad_config.get('train_pose_pred_net', False):
            pose_params = []
            for name, p in self.model.named_parameters():
                if "pose_pred_net" in name:
                    p.requires_grad = True
