...
from transformers import LlavaNextVideoForConditionalGeneration, AutoProcessor,LlavaNextVideoProcessor
...
access_token = "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"
...
class DiffusionEngine(pl.LightningModule):
    def __init__(
        ...
        llava_video_prompt: str = "Describe this video precisely.",
        ground_truth_reference: str = "The robot arm picked up the yellow cube and placed it on the red platform.",
    ):
        super().__init__()
        ...
        self.llava_video_prompt = llava_video_prompt
        self.ground_truth_reference = ground_truth_reference
        
        # LLaVA model and processor (we will initialize them on demand/to CPU first)
    ...
    def _init_llava_model(self, target_device: torch.device):
        """Initializes LLaVA model and processor."""
        if self.llava_model is not None:
            if next(self.llava_model.parameters()).device == target_device:
                return
        
        print(f"Loading LLaVA-NeXT-Video: {self.llava_model_id} on {target_device}...")

        print("access_token",access_token)
        try:
            # self.llava_processor = LlavaNextVideoProcessor.from_pretrained("llava-hf/LLaVA-NeXT-Video-7B-hf", token=access_token)
            ...
            self.llava_model = LlavaNextVideoForConditionalGeneration.from_pretrained(
                self.llava_model_id,
                torch_dtype=torch.float16, # half precision is good
                low_cpu_mem_usage=True,
                token=access_token,
                # *** ADDING DEVICE_MAP="auto" HERE IS CRITICAL ***
                device_map="auto" 
            )

            # Load the processor (it's CPU-based and doesn't need to be moved)
            self.llava_processor = LlavaNextVideoProcessor.from_pretrained(
                self.llava_model_id, 
                token=access_token
            )

            # self.llava_processor = AutoProcessor.from_pretrained( self.llava_model_id, token=access_token )
            # self.llava_processor = LlavaNextVideoProcessor.from_pretrained(self.llava_model_id)
            # self.llava_processor = LlavaNextVideoProcessor.from_pretrained(self.llava_model_id,token=access_token)
    ...
    @torch.no_grad()
    def _llava_bert_score(self, latents: torch.Tensor, B: int, T: int, down_hw: int, reference_captions: List[str] ) -> torch.Tensor:
        """
        ... (docstring remains the same)
        
        Args:
            ... (existing args)
            reference_captions: List[str] of ground truth task descriptions (size B).
        Returns:
            torch.Tensor: [B] tensor of BERTScore F1 values.
        """
        device = latents.device
        self._init_llava_model(device)
        
        # 1. Decode latents to CPU frames in [0, 1]
        imgs_cpu = self._decode_to_cpu_in_chunks(latents, B, T, down_hw=down_hw, chunk=self.reward_decode_mb)

        # 2. Sample frames for LLaVA
        num_llava_frames = 8
        indices = np.linspace(0, T - 1, num_llava_frames).astype(int)
        
        f1_scores = []

        print("_llava_bert_score","B",B)

        for b in range(B):
            # ... (frame sampling and preparation remains the same)
            sampled_frames_float = imgs_cpu[b, indices].permute(0, 2, 3, 1).numpy()
            sampled_frames_uint8 = (sampled_frames_float * 255).astype(np.uint8)
            # Define chat history
            # ... (remains the same)
            conversation = [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": self.llava_video_prompt},
                        {"type": "video"},
                    ],
                },
            ]
            ...
            print("prompt",prompt)
            # 3. LLaVA Inference
            # ... (LLaVA inference logic remains the same)
            inputs_video = self.llava_processor(
                text=prompt, 
                videos=sampled_frames_uint8, 
                padding=True, 
                return_tensors="pt"
            ).to(device, dtype=torch.float16) # Run inference on float16
            print("inputs_video",inputs_video )

            # if "past_key_values" in inputs_video:
            #     pkv = inputs_video["past_key_values"]
            #     if pkv is not None and hasattr(pkv, "__len__") and len(pkv) == 0:
            #         inputs_video["past_key_values"] = None
            # inputs_video["use_cache"] = True
            
            # inputs = dict(inputs_video)  # copy to avoid side-effects
            # inputs.setdefault("use_cache", True)
            # # explicit None to prevent weird empty-cache objects
            # inputs.setdefault("past_key_values", None)
         
            # output = self.llava_model.generate(**inputs_video, max_new_tokens=100)
            ...
            llava_candidate_answer = self.llava_processor.decode(output[0][2:], skip_special_tokens=True).strip()
            llava_candidate_answer = processor.decode(output[0][2:], skip_special_tokens=True)

            print("llava_candidate_answer","$"+llava_candidate_answer+"$")
            # 4. Compute BERTScore (using the batch-specific reference)
            candidate = llava_candidate_answer
            # Use the B-th item from the passed list
            reference = reference_captions[b]
            print("candidate,reference",candidate,reference)
            
            # BERTScore (P, R, F1 are all 1D tensors of size 1)
            P, R, F1 = score([candidate], [reference], lang="en", verbose=False)
            
            f1_scores.append(F1.item())

            print("bert score == ", F1.item())
            
        # ... (LLaVA model offloading remains the same)
        
        return torch.tensor(f1_scores, dtype=torch.float32, device=device)
    ...
    def get_rl_loss(
        ...
        """
        Calculates the DDPO RL loss term using LLaVA + BERTScore as the reward.
        """
        import torch
        # ... (rest of the local helper functions from the original code remain the same)
        # ... (basics / config from the original code remain the same)

        # ------------------- basics / config -------------------
        device = input['video'].device
        dtype= input['video'].dtype
        add_inputs = batch if isinstance(batch, dict) else {}
        # ... (rest of config parsing remains the same)
        ...
        ddpo_lambda = float(getattr(self, "ddpo_lambda", 0.1)) # Use ddpo_lambda
        # ... (rest of config parsing remains the same)
        seq_mb_rollout = max(1, int(getattr(self, "rollout_seq_mb", 2)))
        seq_mb_ddpo    = max(1, int(getattr(self, "ddpo_seq_mb", 2)))
        decode_mb      = max(1, int(getattr(self, "reward_decode_mb", 6))) # Use decode_mb for LLaVA decoding chunk size
        down_hw        = int(getattr(self, "reward_down_hw", 224)) # Use down_hw for LLaVA input resize
        # mid_idx and amp_dtype are still defined but not used for LLaVA reward
        
        # ... (B, T_video, T_pose inference remains the same)
        BxT_v = int(input['video'].shape[0])
        T_v = T_v_decl
        B   = BxT_v // T_v
        # ... (rest of B, T_v, T_p inference and sigmas/inputs setup remains the same)
        
        # NEW: 1. Extract the ground truth task descriptions for the batch (B items)
        reference_captions = self._extract_prompts(batch, B, BxT_v, T_v)
        
        if not all(reference_captions):
            # Fail gracefully if conditioning text isn't found
            print("[WARNING] Could not find task description in batch. RL loss will be zero.")
            return torch.zeros(1, device=device, dtype=torch.float32)

        # ------------------- DDPO Rollout -------------------
        # NOTE: The rollout logic remains largely the same, but for brevity, 
        # I'll only show the key parts where the loss is computed.
        # You would keep the original DDPO rollout logic here, which generates 
        # `x_prevs_list`, `alphas_cumprod_list`, `noise_preds_list`, `x_start_preds_list`.

        # ... (DDPO rollout logic to get x_prevs_list, alphas_cumprod_list, etc. as in original code)
        
        # ------------------- Reward Calculation (LLaVA + BERTScore) -------------------
        
        # Take the final frame (or a specific set of frames) for reward scoring
        # Since we are using LLaVA-Video, we must decode ALL video frames T_v.
        # We use the final predicted video latent state (x_start_preds from last step)
        
        # Get the full video sequence latent prediction (B*T_v, C, H, W)
        # Note: DDPO rollout predicts x_start, not x_t, so we should use the last x_start prediction.
        
        if 'x_start_preds' in locals() and len(x_start_preds_list) > 0:
             # Take x_start from the last DDPO step for the reward
            final_latents = x_start_preds_list[-1]
        else:
            # Fallback (e.g., if steps=1 and you only have the model_output from the first step)
            # This is complex, so for a full video reward, we'll assume the full rollout happens.
            # If a proper x_start isn't available, we'll use the un-noised video input as a placeholder
            # to avoid crashing, though the RL step won't work correctly.
            print("[WARNING] DDPO rollout did not generate x_start_preds. Using un-noised input for reward.")
            final_latents = input['video'] 
        
        # 1. Compute the LLaVA + BERTScore reward for the full video (BxT_v latents)
        # reward_scores is a [B] tensor of BERTScore F1 values
        print("reward_scores")
        reward_scores: torch.Tensor = self._llava_bert_score(
        ...
        # 2. Normalize and compute advantages
        # DDPO uses 'advantages' for the RL loss. The reward is scaled/normalized.
        
        # Convert rewards to float32 if necessary
        rewards = reward_scores.to(dtype=torch.float32)

        # Update running baseline/EMA for advantages
        self.adv_ema.update(rewards.mean().item())
        baseline = self.adv_ema.value
        
        # Calculate advantages (the difference from the running mean baseline)
        advantages = rewards - baseline
        # Simple Advantage normalization (optional, but helps stabilization)
        adv_mean = advantages.mean()
        adv_std  = advantages.std()
        advantages = (advantages - adv_mean) / (adv_std + 1e-8)
        
        # ------------------- DDPO RL Loss Calculation -------------------
        
        # Calculate DDPO loss (KL divergence between the current policy and the rollout policy)
        rl_loss = self._ddpo_loss_from_rollout(
            advantages=advantages,
            x_prevs_list=x_prevs_list, # Latent at time t
            alphas_cumprod_list=alphas_cumprod_list,
            noise_preds_list=noise_preds_list,
            x_start_preds_list=x_start_preds_list,
            steps=steps,
            eta=eta,
            network=network,
            denoiser=denoiser,
            cond=cond,
            input=input, # for BxT slicing
            BxT_v=BxT_v, # for BxT slicing
            BxT_p=BxT_p, # for BxT slicing
            B=B,
            T_v=T_v,
            T_p=T_p,
            seq_mb_ddpo=seq_mb_ddpo, # sequences per DDPO chunk
            amp_dtype=amp_dtype,
        )

        # 3. Apply lambda weight
        total_rl_term = ddpo_lambda * rl_loss
        
        # Optional: Log the reward components
        self.log("ddpo/reward_mean", rewards.mean(), on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
        self.log("ddpo/reward_max", rewards.max(), on_step=True, on_epoch=False, logger=True, sync_dist=True)
        self.log("ddpo/advantage_mean", advantages.mean(), on_step=True, on_epoch=False, logger=True, sync_dist=True)
        self.log("ddpo/rl_loss", rl_loss, on_step=True, on_epoch=False, logger=True, sync_dist=True)
        
        return total_rl_term
    ...
    def forward(self, x, batch):
        ...
        print("get_rl_loss")
        loss_total = self.get_rl_loss(loss_mean, self.model, self.denoiser, self.conditioner, x, batch, sigmas, noised_input, model_output) + loss_mean
